...
- 요구사항 명세 기법
  ...
  - 비정형 명세 기법은 사용자의 요구를 표현할 때 Z 비정형 명세 기법을 사용한다. (X)
    - Z는 정형 명세 기법이다.
...
- 요구 분석(Requirement Analysis)
  ...
  - 도메인 분석(Domain Analysis)은 요구에 대한 정보를 수집하고 배경을 분석하여 이를 토대로 모델링을 하게된다.
  - 기능적(Functional) 요구에서 성능, 보안, 품질, 안정 등에 대한 요구사항을 도출한다.(X)
    - 보안, 품질 등은 비기능 요구사항이다.
...
- 요구사항 분석의 어려움
  ...
  - 사용자의 요구는 예외가 거의 없어 열거와 구조화가 어렵지 않다. (X)
    - 수시로 예외가 발생할 수 있어 열거와 구조화가 어렵습니다.
...
- 요구사항 검증(Requirements Validation)
  ...
  - 요구사항 검증 과정을 통해 모든 요구사항 문제를 발견할 수 있다. (X)
    - 사용자의 요구는 수시로 예외가 발생할 수 있어 검증 과정에서도 모든 요구사항 문제를 발견하기란 쉽지 않다.
